...
====================
GPU solver for batched tridiagonal systems, written as a C function that runs directly on the GPU.
One GPU thread handles one complete tridiagonal solve, so a batch of B simulation runs is solved in parallel.

What system is being solved: at each Crank-Nicolson time step the solver in
column_solver.py produces a tridiagonal system A * theta = d over the 221 depth
nodes. It is tridiagonal because central finite differences couple each node
only to its two neighbours.

The Thomas algorithm solves such a system in a single forward sweep and a single
back substitution, linear in the number of nodes, where a general dense solve is
cubic. Writing it as one C function keeps the whole 221-node solve inside the
kernel, so a batch of thousands of runs costs one GPU launch rather than
hundreds of small ones.

If no compatible GPU is found or the C code cannot be compiled, the module falls
back to a pure CuPy/NumPy solver and still works, at the cost of speed.
...
Changelog
---------
2026.09.07  v1.2  Repository release. Renamed from thomas_kernel.py. Header
                  rewritten to state what the algorithm buys rather than what
                  it replaced. No code changed.
2026.07.28  v1.1  Docstring note on the Crank-Nicolson system this solves.
                  Comment only, no code changed.
2026.06.09  v1.0  Initial implementation, verified against scipy to machine
                  precision.
"""

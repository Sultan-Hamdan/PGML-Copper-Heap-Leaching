...
DEFINITION
  R_stable  the largest rate for which one day from the driest state the
            controller will ever see produces zero pre-clip excursions outside
            [theta_r, theta_s]. The last stable rate, not the first that floods.

  r_max = min(0.30 * Ks, safety * R_stable)

WHY
  The top boundary is a prescribed flux, so it assumes everything applied
  infiltrates. Above the surface's infiltration capacity a real heap ponds and
  sheds the excess, and this scheme has no ponding or runoff term. The solver
  shows it as a Picard iteration returning finite but unconverged values, which
  the clip flattens into a profile that looks physical. Capping keeps the solver
  inside the model's stated domain.

  Day 1 is the worst case: the column starts uniform at Se_i and irrigation only
  adds water, so infiltration capacity only rises from there.

FLAGS
  --split           val | test. Default val.
  --soil_table      JSON of constructed soils, for the extrapolation cells.
  --reference_soil  the Cariaga 2015 soil, which has no dataset entry.
  --se_grid         initial saturations to test. Pass "own" to match the
                    ceiling used for the validation and test soils; any other
                    grid gives a different action set.
  --safety          margin on R_stable. Default 0.95.
  --se_floor        fixed uniform Se instead of each soil's own.
...
controller uses to confirm which ceiling it read.
...
2026.09.07  v2.5  Repository release. Renamed from probe_flood_ceiling_v2.py;
                  R_flood renamed to R_stable; output renamed to
                  rmax_<split>.json; paths routed through paths.py.
2026.08.24  v2.4  NaN counts as a failure. A comparison against NaN returns
                  ...
                  had recorded R_stable = 0.
2026.08.24  v2.3  --soil_table reads constructed soils instead of the dataset.
                  Output is named per cell so ceilings cannot be crossed.
...
2026.08.13  v2.0  --reference_soil for the Cariaga 2015 soil. Uncapped, the
...
2026.08.01  v1.4  Ceiling is the per-soil minimum over several initial
                  saturations, not day 1 alone. The limit is not monotone in
                  wetness, and the day-1 ceiling let 4 of 614 soils diverge.
2026.08.01  v1.3  Safety margin applies only where the stability limit binds.
                  v1.2 shaved 5 percent off the cap for all 64 soils.
2026.08.01  v1.2  Thresholds computed after the sweep. v1.1 used res_tol = 1e-4,
                  which mis-binned 55 of 64 soils. The distribution is bimodal
                  with a six order of magnitude gap.
2026.08.01  v1.1  Criterion is the final Picard pass plus its residual. v1.0
                  counted normal early overshoot as divergence.
...
def main():
    ...
    if a.soil_table:
        ...
        # closed_loop.py compares its ceiling's id list against the soil

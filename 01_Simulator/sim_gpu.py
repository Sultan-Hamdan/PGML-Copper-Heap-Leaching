...
sim_gpu.py
==========
GPU batch driver for the VGM simulator. Runs many independent simulation runs
in parallel on the GPU, producing the same Se output as sim_column.py but
across a full batch of B runs at once.

The hydraulic functions (S_theta, h_theta, K_theta, D_theta, dtheta_dh_theta)
are intentionally re-implemented inline here rather than imported from
hydraulics.py. This is because hydraulics.py uses NumPy, and all arrays here
are CuPy arrays when running on GPU. Importing from hydraulics.py would
silently pull data back to CPU, defeating the purpose of GPU batching. Do not
refactor this into a shared import without accounting for this.

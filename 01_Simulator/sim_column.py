...
Port of the serial VGM simulator:
    ...
    columna.m     -> simulate_day()     (one day: 24 hourly sub-steps, 20 inner
                                         fixed-point iterations each)

run_simulation() runs a full simulation over tf days, advancing the moisture
profile hourly, and returns the complete Se and theta histories alongside
downsampled sensor readings. All input parameters (soil properties, grid
settings, irrigation schedule) are bundled in a SimParams object so that
generating a dataset is just calling run_simulation() once per parameter set.

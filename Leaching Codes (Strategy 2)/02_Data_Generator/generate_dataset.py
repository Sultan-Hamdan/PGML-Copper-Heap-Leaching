...
generate_dataset.py
===================
Pipeline driver for dataset generation. Reads N Q-schedules from existing
MATLAB .mat files, runs all N simulations through the GPU batch runner in one
pass, and saves the results to a single HDF5 file. The strategy label (and so
the output filename) is set by --strategy; N is however many run_*.mat files
are found.

Steps:
    1. Read Q-schedules from run_XXXX.mat files (via --mat_dir)
    2. Run all schedules through sim_gpu.run_batch() in one GPU batch
    3. Save all runs into strategy<S>.h5 (one group per run)
    4. Print a timing summary and sanity check on first and last run

SOIL PARAMETERS:
    Ks, alpha, n, theta_i are fixed Cariaga values (S1 and S2 both use fixed
    soil; only the Q-schedules and theta_i vary). For S3, soil varies per run,
    so these module-level constants become per-run arrays passed into run_batch
    and into compute_r_out / compute_q_out. That is the S3 change point.
...
Changelog
---------
2026.07.22  v1.1  Made strategy-agnostic: added --strategy (default 1) so the
                  output filename strategy<S>.h5 and the file-level strategy attr
                  are set at the CLI instead of hardcoded to 1. Generic docstring
                  wording (N runs, not 201). No physics change; soil still fixed
                  (S1/S2), with the per-run S3 change point noted above.
...
def main(mat_dir, out_dir, strategy):
    ...
    h5_path = save_all_h5(out_dir, names, Se_all, Q_batch, strategy=strategy)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir", required=True,
                        help="Output folder for the strategy<S>.h5 output")
    parser.add_argument("--strategy", default="1",
                        help="Strategy label S; sets output name strategy<S>.h5 "
                             "and the file-level strategy attr. Default 1.")
    args = parser.parse_args()
    main(args.mat_dir, args.out_dir, args.strategy)

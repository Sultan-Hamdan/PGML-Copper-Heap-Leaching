...
producing the same Se output as 00_MATLAB_Ports/sim_column.py but across a full batch of B runs at once.

The hydraulic functions (S_theta, h_theta, K_theta, D_theta, dtheta_dh_theta) are intentionally re-implemented inline
here rather than imported from 00_MATLAB_Ports/hydraulics.py.
That file uses NumPy, and all arrays here are CuPy arrays when running on GPU.
Importing it would silently pull data back to CPU, defeating the purpose of GPU batching.
...
For dataset generation with varied soil parameters (Ks, alpha, n, theta_i),
pass them as arrays of length B instead of scalars and the physics will apply
each parameter set to its corresponding run automatically.
...
Changelog
---------
2026.07.19  v1.2  Added optional theta0 to run_batch (default None, path
                  unchanged). When given, the run starts from a supplied
                  (B, Nz+1) profile instead of the uniform theta_i initial
                  condition. Needed for one-day-at-a-time closed-loop stepping.
                  ASCII-only.
2026.07.19  v1.1  Added optional return_qout to run_batch (default False, so
                  all existing callers are unaffected). When True it also
                  returns the hourly outlet-flow proxy (B, tf*D_H), computed
                  from the end-of-hour bottom-node conductivity exactly as
                  columna.m. Added daily_qout() helper to collapse it to the
                  daily average proxy (q_out_proceso) the identifier needs.
                  ASCII-only. No physics or default-path behaviour changed.
2026.06.09  v1.0  Initial GPU batch implementation, verified vs 00_MATLAB_Ports/sim_column.py
"""
...
def run_batch(Q_batch, Ks=170.0, theta_s=0.33, theta_r=0.0, theta_i=0.14,
    ...
    # Hourly outlet-flow proxy, bottom node. Mirrors columna.m:
    ...
    for dia in range(tf):
        ...
        for h in range(D_H):
            ...
            if return_qout:
                # End-of-hour bottom-node conductivity, exactly as columna.m.
...
def daily_qout(qout_full, tf, D_H=24):
    ...
    in the MATLAB main script (q_out_proceso)."""
    B = qout_full.shape[0]

...
The matrix and boundary condition assembly is mirrored line-for-line against
the MATLAB, with the only change being the unavoidable 1-based to 0-based
index shift:
...
build_A_c() returns the three diagonals (lower, main, upper) plus c, so the
same assembly feeds both solvers with no risk of them diverging because of
differing assembly code.

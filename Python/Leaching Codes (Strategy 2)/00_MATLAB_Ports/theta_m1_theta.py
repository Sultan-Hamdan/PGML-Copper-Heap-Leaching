...
theta_m1_theta.py
=================
Port of theta_m1_theta.m, computes a virtual node one step above the physical top of the column (z = 0),
used to enforce the irrigation/infiltration boundary condition.
Because the finite-difference formula at the top node requires a neighbour above it that does not physically exist,
a virtual node (called a ghost node in numerical PDE literature) is constructed from the boundary condition itself.

This function is mirrored statement-by-statement against the MATLAB rather than restructured,
because the boundary condition assembly is sensitive to index ordering and sign conventions.
A small mistake produces wrong results silently with no error raised.

The ghost-node water content theta_{-1} is found by rearranging the top boundary condition (Eq. 6 in the Olivares formulation):
...
The hydraulic properties (K, D) are then re-evaluated at this ghost node so the finite-difference formula at the top node can use them.

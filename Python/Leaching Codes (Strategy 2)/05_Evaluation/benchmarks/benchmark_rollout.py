...
Chains 600 forward steps (25 days x 24 hours) two ways, from the same initial state and the same constant irrigation schedule:

    PGML      : 600 successive neural-net passes (float32, PyTorch) output of each step fed back as input to the next
...
Two results are reported:
    ...
    Speed : total wall time for the full 600-step chain, mean over reps, plus speedup factor (sim time / PGML time).

    Drift : after both rollouts, mean absolute error between PGML Se and sim Se at each of the 600 timesteps.
            Reports max drift over the run and the timestep at which it peaks.
            Drift is computed for B=1 only (representative; heap-scale drift is the same per column).
...
Lambda selection (drift only, B=1, ranked over a folder of checkpoints):
    python benchmark_rollout.py --select ..\\04_Training\\convex_sweep
...
Changelog
---------
2026.07.22  v1.2  Added a lambda-SELECTION mode (--select DIR). Loops over every
                  checkpoint in DIR, runs the B=1 open-loop drift only (no speed
                  sweep), and prints one table ranked over lambda: peak MAE
                  (worst excursion), final MAE (settle), mean MAE. The simulator
                  truth is computed once and reused for all checkpoints (fixed
                  Q/soil/geometry, so the reference is identical). Flags the
                  lowest-settle and lowest-peak lambda so the pick is made on the
                  same open-loop basis as the additive sweep. Existing speed+drift
                  behaviour is unchanged when --select is omitted.
2026.07.18  v1.1  Added a [0,1] clamp on the fed-back Se inside the free-run
                  loop (Se is effective saturation, physically bounded). Without
                  it a single out-of-range prediction feeds back and the chain
                  diverges (diagnosed on pgml_lam10: Se reached -1.30 by step 37,
                  MAE blew up to 1.19). Clamp is on by default; --no_clamp
                  reproduces the old unbounded behaviour. Drift now measures the
                  model's real self-feeding error, not an unphysical blow-up.
...
# =============================================================================
def pgml_rollout(model, Se_init, R_scaled, soil_vec, device, clamp=True):
    B = Se_init.shape[0]
    ...
    with torch.no_grad():
        for t in range(N_STEPS):
            ...
            # Se is effective saturation, physically bounded to [0,1]. Without
            # this clamp a single out-of-range prediction feeds back into the
            # next input and the free-run chain diverges (Se goes negative,
            # error self-amplifies). Clamping keeps the fed-back state physical
            # so the drift measured is the model's, not an unbounded blow-up.
            if clamp:
                Se_cur = Se_cur.clamp(0.0, 1.0)
            traj[:, :, t] = Se_cur
...
# =============================================================================
# Lambda selection: rank a folder of checkpoints on open-loop drift (B=1).
# Reads the lambda from the checkpoint name (e.g. pgml_lam0p4.pt -> 0.4,
# mlp_lam0.pt -> 0.0). Same fixed Q/soil/geometry as the drift benchmark, so
# the simulator truth is computed once and reused for every checkpoint.
# =============================================================================
...
def run_selection(select_dir, scaler_path, device, clamp):
    ckpts = sorted(glob.glob(os.path.join(select_dir, "*.pt")))
    if not ckpts:
        print(f"ERROR: no .pt checkpoints found in {select_dir}. Aborting.")
        return

    print("=" * 78)
    print("LAMBDA SELECTION  --  open-loop drift (B=1), ranked over checkpoints")
    print("=" * 78)
    print(f"Device        : {device}")
    print(f"Checkpoints   : {len(ckpts)} in {select_dir}")
    print(f"Steps         : {N_STEPS}  ({SIM['tf']} days x {SIM['D_H']} h)")
    print(f"Q schedule    : constant {Q_CONST:.1f} L/h  -> R = {R_CONST:.6f} cm/h")
    print(f"Se clamp      : {'ON [0,1]' if clamp else 'OFF'}")

    if not os.path.isfile(scaler_path):
        print(f"ERROR: scaler.json not found at {scaler_path}. Aborting.")
        return
    scalers = load_scalers(os.path.dirname(scaler_path))
    R_scaled, soil_vec = build_pgml_inputs(scalers)

    # simulator truth once (identical setup for every checkpoint)
    Se_sim = sim_rollout(1)[0].astype(np.float64)          # (221, 600)

    Se0_val = (SIM["theta_i"] - SIM["theta_r"]) / (SIM["theta_s"] - SIM["theta_r"])
    Se_init = torch.full((1, NZ1), Se0_val, device=device, dtype=torch.float32)

    rows = []
    for path in ckpts:
        lam = parse_lambda_from_name(path)
        model = load_pgml(path, device)
        Se_pgml = pgml_rollout(model, Se_init, R_scaled, soil_vec, device,
                               clamp=clamp)[0].astype(np.float64)   # (221, 600)
        mae_t = np.mean(np.abs(Se_pgml - Se_sim), axis=0)           # (600,)
        rows.append({
            "lam":   lam,
            "name":  os.path.basename(path),
            "peak":  float(mae_t.max()),
            "pstep": int(mae_t.argmax()),
            "final": float(mae_t[-1]),
            "mean":  float(mae_t.mean()),
        })

    # column minima (mark the best in each)
    min_peak  = min(r["peak"]  for r in rows)
    min_final = min(r["final"] for r in rows)
    min_mean  = min(r["mean"]  for r in rows)

    rows.sort(key=lambda r: (r["lam"] if r["lam"] is not None else 1e9))

    print("\n" + "-" * 78)
    print(f"{'lambda':>7}  {'checkpoint':<18}{'peak MAE':>13}{'(step)':>8}"
          f"{'final MAE':>13}{'mean MAE':>13}")
    print("-" * 78)
    for r in rows:
        lam_s = f"{r['lam']:.2f}" if r["lam"] is not None else "  ? "
        peak_s  = f"{r['peak']:.4e}"  + ("*" if r["peak"]  == min_peak  else " ")
        final_s = f"{r['final']:.4e}" + ("*" if r["final"] == min_final else " ")
        mean_s  = f"{r['mean']:.4e}"  + ("*" if r["mean"]  == min_mean  else " ")
        print(f"{lam_s:>7}  {r['name']:<18}{peak_s:>13}{r['pstep']:>8}"
              f"{final_s:>13}{mean_s:>13}")
    print("-" * 78)
    print("* = best (lowest) in that column.  peak = worst excursion, "
          "final = settle, mean = overall.")

    # picks
    best_settle = min(rows, key=lambda r: r["final"])
    best_peak   = min(rows, key=lambda r: r["peak"])
    print("\nLowest settle (final MAE) : lambda "
          f"{best_settle['lam']:.2f}  ({best_settle['final']:.4e}, "
          f"peak {best_settle['peak']:.4e})")
    print("Lowest peak (safest)      : lambda "
          f"{best_peak['lam']:.2f}  ({best_peak['peak']:.4e}, "
          f"final {best_peak['final']:.4e})")
    if best_settle["lam"] == best_peak["lam"]:
        print("-> Same lambda wins settle AND peak: clean pick.")
    else:
        print("-> Settle and peak favour different lambdas. Pick on settle, "
              "then confirm the peak is acceptable (safety), as in the additive sweep.")
    print("=" * 78)
...
def main():
    ...
    p.add_argument("--ckpt",
                   default=os.path.join(_HERE, "..", "06_Control", "pgml_lam0p4.pt"))
    ...
    p.add_argument("--no_clamp", action="store_true",
                   help="Disable the [0,1] clamp on fed-back Se (reproduces "
                        "the unbounded free-run blow-up). Default: clamp on.")
    p.add_argument("--select", default=None,
                   help="Lambda-selection mode: path to a folder of checkpoints. "
                        "Runs B=1 drift only and prints a ranked table (no speed "
                        "sweep). Default: off (runs the normal speed+drift bench).")
    args = p.parse_args()
    ...
    # --- selection mode: rank a folder of checkpoints on drift, then exit ---
    if args.select is not None:
        run_selection(args.select, args.scaler, tdev, clamp=not args.no_clamp)
        return
    ...
    print(f"Se clamp         : {'OFF (--no_clamp)' if args.no_clamp else 'ON [0,1]'}")
    ...
    for B in batches:
        ...
        if B == 1:
            ...
            drift_Se_pgml = pgml_rollout(
                model, Se_init, R_scaled, soil_vec, tdev,
                clamp=not args.no_clamp)  # (1, 221, 600) float32

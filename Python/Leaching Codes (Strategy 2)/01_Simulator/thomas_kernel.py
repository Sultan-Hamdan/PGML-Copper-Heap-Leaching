...
The reason for writing this in C rather than using CuPy's built-in banded solver (cupyx.scipy.linalg.solve_banded)
is that our earlier implementation looped over 221 depth nodes in Python, launching a small GPU operation at every step.
With thousands of runs this produced millions of small GPU calls, which is very slow.
The C function here does the entire 221-node solve in one go with no Python overhead between steps.

If no compatible GPU is found or the C code cannot be compiled, the module falls back to a pure CuPy/NumPy solver and still works,

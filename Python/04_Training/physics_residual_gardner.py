...
===========================
The PGML physics loss with Gardner hydraulics. A Gardner swap of the verified
VGM baseline physics_residual.py: ONLY the hydraulics (K, D) and the TOP
boundary condition change. Everything structural -- the three-point stencil,
the weights w1/w2, the face-averaging, the b/A/c assembly and the residual form
res = A @ theta_pred - (b + c) -- is identical to the VGM version.
...
Gardner uses its OWN parameters alpha_g, m_g (Olivares offline fit: ~4.0047,
~16.4094), DIFFERENT from VGM's alpha, n. They are explicit inputs
(future-proof for Strategy 3, where soil varies per run).
...
TOP BOUNDARY -- why it differs from VGM (the fix):
    ...
    This DIVIDES by D0. Gardner's D is tiny (~1e-6..1e-9), so the ghost
    explodes (Se**16 -> residual ~1e+130 on any R>0 step). Instead we use
    Gardner's own surface condition "flux = R" and substitute the KNOWN flux
    R directly into the stencil, eliminating the ghost theta entirely:
        Db*(theta_0 - theta_m1)  ->  Dz * (K0 - R)     (no 1/D anywhere)
        K_{-1}                    ->  K0                (surface advective value)
    Matrix consequence: lower0 = 0 (no ghost coupling), the backward face moves
    to the RHS (c0), main0/upper0 keep only the forward face. Derived from the
    dimensional Gardner flux balance (sympy), verified numerically on run_0000.
...
VETTING MINDSET (different from the VGM version):
    ...
    (the gap between the VGM truth and the Gardner model). We verify:

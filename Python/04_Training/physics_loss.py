...
Same maths, made training-ready: it runs a whole BATCH of runs at once on the
GPU, and gradients flow through it so the physics term can steer the network.
...
Everything physical is identical: Gardner K/D, the stencil, w1/w2, the
top-boundary flux fix (ghost eliminated, no division by D0), bottom zero-grad,
and res = A @ theta_pred - (b + c).

Gardner params m, alpha_g are explicit inputs (fixed for Strategy 1; per-run
for Strategy 3). Soft-penalty use: total = data_loss + lam * physics_loss.

VETTING: the torch result MUST match the verified NumPy result to machine
precision (single run AND each row of a batch). That equivalence is the safety
net -- if they diverge, the torch port has a bug.

...
Runs BOTH trained surrogates (mlp_best.pt, pgml_best.pt) on the held-out TEST
set and reports TWO axes, the comparison val loss alone cannot show:

    data error      = MSE( y_pred, VGM truth )        <- what val loss sees
    physics residual = mean( Gardner residual^2 )      <- what val loss misses

Why two axes (the whole point): the MLP fits data tighter, but it is never
constrained to obey the physics. The physics residual is the axis where PGML is
expected to win. The 2x2 (model x axis) is the thesis claim made measurable.
...
THREE OUTPUTS (one computation, saved at three levels of detail):
  ...
  2. PER-DEPTH PROFILE -> mean squared residual per node (221), saved as a PNG
        (shows WHERE each model violates physics - expected worst at the top
         surface-flux node and across a sharp wetting front)
...
GROUNDING (nothing physical re-derived here):
  - physics_loss / physics_residual_gardner_torch : VERIFIED (matches the NumPy
    reference to ~5e-17; the NumPy reference was verified on real run_0000 data).
...
FAIR-COMPARISON NOTE: both models are scored with the identical physics_loss
...
2026.06.15  v1.2  Added --mlp and --pgml args so checkpoints are named
                  ...
                  sweep_ckpts/mlp_best.pt (lam=0) and pgml_lam100.pt (lam=100).

...
lambda selects which model is trained, using identical machinery (same MLP, clipping, scheduler, data)
so the only difference is the physics term -- a fair PGML-vs-baseline comparison:
...
def run_epoch(model, loader, data_criterion, optimizer, device,
    ...
    with context:
        for x_batch, y_batch in loader:
            ...
            loss   = data_l + lam * phys_l                                  # 4. THE combined objective

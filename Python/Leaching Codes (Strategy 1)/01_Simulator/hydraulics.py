...
These are pure elementwise relations, so they are written as NumPy vector operations,
meaning each function operates on whole arrays at once rather than looping node by node.
This is mathematically identical to the MATLAB per-element loops and is compatible with GPU batch processing in sim_gpu.py.

Convention: `method == 1` selects van Genuchten (VG); anything else selects the VGM variant used in this codebase,
exactly as the MATLAB does.
...
# -----------------------------------------------------------------------------
if __name__ == "__main__":  # Standalone sanity check
    # run_0000 parameters
    theta_s = 0.33
    theta_r = 0.0

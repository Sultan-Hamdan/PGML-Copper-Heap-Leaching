...
This is the like-for-like forward-model comparison: PGML was trained to reproduce exactly this hour-step.
Accuracy (does PGML match the VGM Se) is settled elsewhere (evaluate.py); this script measures SPEED only.

Reported in seconds first, then as an x-times factor, single column and batched across many columns (the heap-scale view).
Precision is native on each side (simulator float64, PGML float32) because that is how each is actually run
the gap is stated, not hidden.

DOES NOT EDIT sim_gpu.py. It imports sim_gpu's existing functions and calls them in the same order the hourly step already uses.
The data-generation code is untouched.

Fairness controls: per-framework warm-up (exclude cold start and kernel compile), explicit GPU synchronize around every timed call
(GPU calls are async, so naive timing reads wrong), many repeats, report mean / median / std / min.
